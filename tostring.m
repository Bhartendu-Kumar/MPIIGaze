...
% is true. It is false by defualt.
    ...
    for i = iterator
       ...
       minWidth = ceil(1.5*parentWidth / nSiblings);
    ...
    for i = 1 : numel(iterator)
        ...
        % Line 2: the content	
	if isstruct(obj.get(ID)) == 0
	   contentStr = strContentTree.get(ID);
	   contentWidth = numel(contentStr);
           dc = floor(contentWidth / 2);%14
	   str(row, index-dc : index + contentWidth-1-dc) = contentStr;
	else
	   data=obj.get(ID);
	   str1= strcat('samples:',num2str(data.num) );
	   str2= strcat('m:(',num2str(data.mean(1)),',',num2str(data.mean(2)),')');
	   str3= strcat('std:(',num2str(data.std(1)),',',num2str(data.std(2)),')');
	   contentWidth = numel(str1);
           dc = floor(contentWidth / 2);%14
	   str(row, index-dc : index + contentWidth-1-dc) = str1;
	   contentWidth = numel(str2);
           %dc = floor(contentWidth / 2);%14
	   str(row+1, index-dc : index + contentWidth-1-dc) = str2;
	   contentWidth = numel(str3);
	   str(row+2, index-dc : index + contentWidth-1-dc) = str3;
